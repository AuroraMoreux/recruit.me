namespace RecruitMe.Services.Data.Tests
{
    ...
    public class JobSectorsServiceTests
    {
        [Fact]
        ...
        public async Task DeleteMarksRecordAsDeleted()
        {
            AutoMapperInitializer.InitializeMapper();
            var context = InMemoryDbContextInitializer.InitializeContext();
            await context.JobSectors.AddAsync(new JobSector { Id = 1, Name = "Category", IsDeleted = false });
            ...
            var dbRecord = await context.JobSectors.FindAsync(1);
            Assert.True(result);
        ...
        private IEnumerable<JobSector> SeedData()
        {
